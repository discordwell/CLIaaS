import type {
  Ticket, Message, Customer, Organization, KBArticle, Rule, ExportManifest,
} from '../schema/types';
import {
  createClient, paginateOffset, paginateCursor, setupExport, appendJsonl, writeManifest, exportSpinner,
  initCounts, fuzzyStatusMatch, fuzzyPriorityMatch,
...
// ---- Client ----
// Uses createClient with responseMiddleware for session capture and
// errorHandler for Kayako MFA (403 OTP_EXPECTED).

function createKayakoClient(auth: KayakoAuth): ConnectorClient {
  return createClient({
    ...
    errorHandler: (res, body) => {
      ...
      if (res.status === 403 && body.includes('OTP_EXPECTED')) {
        return new Error('Kayako account requires MFA (2FA). Please disable MFA for the API user or use OAuth.');
      }
...
/**
 * @deprecated Use createKayakoClient() + client.request() instead. Kept for backward compatibility.
 * This wrapper preserves the original kayakoFetch(auth, path, options) signature.
 */
export async function kayakoFetch<T>(auth: KayakoAuth, path: string, options?: {
  method?: string;
  body?: unknown;
}): Promise<T> {
  return createKayakoClient(auth).request<T>(path, options);
}
...
export async function kayakoUpdateCase(auth: KayakoAuth, caseId: number, updates: {
  ...
  await createKayakoClient(auth).request(`/api/v1/cases/${caseId}.json`, {
    method: 'PATCH',
    body,
  });
}
...
export async function kayakoPostReply(auth: KayakoAuth, caseId: number, contents: string): Promise<void> {
  await createKayakoClient(auth).request(`/api/v1/cases/${caseId}/reply.json`, {
    method: 'POST',
    body: { contents },
  });
}
...
export async function kayakoPostNote(auth: KayakoAuth, caseId: number, bodyText: string): Promise<void> {
  await createKayakoClient(auth).request(`/api/v1/cases/${caseId}/notes.json`, {
    method: 'POST',
...
export async function kayakoCreateCase(auth: KayakoAuth, subject: string, contents: string, options?: {
  ...
  const result = await createKayakoClient(auth).request<{ data: { id: number } }>('/api/v1/cases.json', {
    method: 'POST',
    body,
  });
...
export async function kayakoVerifyConnection(auth: KayakoAuth): Promise<{
  ...
  try {
    const client = createKayakoClient(auth);
    // Test auth by fetching current user (first page of users with limit 1)
    const userData = await client.request<KayakoPaginatedResponse & { data: KayakoUser[] }>(
      '/api/v1/users.json?limit=1',
    );
    const user = userData.data?.[0];

    // Get case count
    const caseData = await client.request<KayakoPaginatedResponse>(
...
export async function exportKayako(auth: KayakoAuth, outDir: string): Promise<ExportManifest> {
  ...
  const fetchFn = client.request.bind(client);
    ...
    onPage: async (cases) => {
      for (const c of cases) {
        const ticket: Ticket = {
          ...
          source: 'kayako',
          subject: c.subject,
          status: fuzzyStatusMatch(getStatusLabel(c.status)),
          priority: fuzzyPriorityMatch(getPriorityLabel(c.priority)),
        ...
        try {
          const notes = await client.request<KayakoPaginatedResponse & { data: Array<{
            id: number;
            body_text?: string;
            user?: { id: number; full_name?: string } | null;
            created_at: string;
          }> }>(`/api/v1/cases/${c.id}/notes.json?limit=100`);
  ...
  try {
    const data = await client.request<KayakoPaginatedResponse & { data: KayakoTrigger[] }>(
