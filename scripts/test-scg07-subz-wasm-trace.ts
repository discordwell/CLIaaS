/**
 * Trace subz team state WASM vs TS tick-by-tick.
 * Use this to verify WASM's "1 recruit per tick" behavior.
 */
import { test } from '@playwright/test';

const BASE_URL = process.env.BASE_URL ?? 'https://cliaas.com';

test('SCG07EA subz WASM vs TS trace', async ({ browser }) => {
  ...
  await Promise.all([
    wp.goto(`${BASE_URL}/ra/original.html?scenario=SCG07EA.INI&autoplay=1&agentharness=1&seed=0`, { waitUntil: 'load' }),
    tp.goto(`${BASE_URL}?anttest=agent&scenario=SCG07EA&difficulty=normal`, { waitUntil: 'load' }),
  ]);
  await Promise.all([
    wp.waitForFunction(() => {
      try {
  ...
  for (let tick = 1; tick <= 6; tick++) {
    const [wRes, _] = await Promise.all([
      wp.evaluate(async () => {
        ...
        const teams = (s.teams ?? []) as Array<{
          ...
          hb: boolean;
          alt: boolean;
          members: Array<{ type: string; have: number; ids: string }>;
        }>;
        return teams.filter(t => t.cls === 'subz').map(t => ({
    ...
    const tRes = await tp.evaluate(() => {
      ...
      return teams.filter((t: any) => t.desiredMembers.some((m: any) => m.type === 'SS')).map((t: any) => ({
        total: t.total,
