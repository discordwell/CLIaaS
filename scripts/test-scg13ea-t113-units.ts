  ...
  for (let t = 112; t <= 115; t++) {
    ...
    const wasmMembers = await wasmPage.evaluate(() => {
      ...
      return all
        ...
        .map((u: { id: number; cx: number; cy: number; m: number; mt: number; mq: number; nlx?: number; nly?: number; doing: number }) =>
          ({ id: u.id, c: `(${u.cx},${u.cy})`, m: u.m, mt: u.mt, mq: u.mq, nav: u.nlx ? `(${u.nlx},${u.nly})` : null, doing: u.doing }));
    });
    ...
    const tsMembers = await tsPage.evaluate(() => {
      ...
      return game.entities
        ...
        .map((e: { id: number; cell: { cx: number; cy: number }; mission: string; missionTimer: number; missionQueue: string | null; moveTarget: { lx: number; ly: number } | null; doing: string; teamRef?: { id: number }; isDriving: boolean }) =>
          ({ id: e.id, c: `(${e.cell.cx},${e.cell.cy})`, m: e.mission, mt: e.missionTimer, mq: e.missionQueue, nav: e.moveTarget ? `(${e.moveTarget.lx},${e.moveTarget.ly})` : null, doing: e.doing, drv: e.isDriving, team: e.teamRef?.id }));
    });
