...
- **Results**: All position diffs eliminated. TRAN 14→0, JEEP 1→0, DOG 6→0 cells. Tick 0 matches perfectly.
...
- **Wave 0 fixes**: Grenade projSpeed test (12→5), TEVENT_DESTROYED (pendingDestroyedCount), campaign spawn scatter tolerance, dead callbacks removed from repairSell/specialUnits/production contexts.
- **Source-string tests updated**: repair-sell-pipeline.test.ts reads from ai.ts, combat.ts, placement.ts, missionAI.ts instead of index.ts.
- **All 115/115 test files pass (14,384 tests)**.
- **Committed**: 1dea8d3, pushed to main, deployed to cliaas.com.
