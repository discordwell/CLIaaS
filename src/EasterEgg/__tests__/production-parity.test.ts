...
describe('RP4: Service depot repair rate (~14 tick interval)', () => {
  ...
  it('repair cost per step calculation is unchanged', () => {
    // Cost formula: ceil((unitCost * REPAIR_PERCENT) / (maxHp / REPAIR_STEP))
    const unitCost = 800; // e.g. medium tank
    const maxHp = 400;
    const totalRepairCost = unitCost * REPAIR_PERCENT; // 200
    const steps = maxHp / REPAIR_STEP; // 80
    const costPerStep = Math.ceil(totalRepairCost / steps); // ceil(2.5) = 3
    expect(costPerStep).toBe(3);
  });

  it('time to repair 100 HP at 14-tick interval is ~19.6 seconds', () => {
...
describe('SL3: Wall selling — instant removal + refund', () => {
  it('SBAG wall sell gives 50% refund (5 credits)', () => {
    const sbag = PRODUCTION_ITEMS.find(p => p.type === 'SBAG');
    expect(sbag).toBeDefined();
    expect(calcSellRefund(sbag!.cost)).toBe(Math.floor(10 * 0.5)); // 5
  });

  it('FENC wall sell gives 50% refund (12 credits)', () => {
    const fenc = PRODUCTION_ITEMS.find(p => p.type === 'FENC');
    expect(fenc).toBeDefined();
    expect(calcSellRefund(fenc!.cost)).toBe(Math.floor(25 * 0.5)); // 12
  });

  it('BARB wall sell gives 50% refund (10 credits)', () => {
    const barb = PRODUCTION_ITEMS.find(p => p.type === 'BARB');
    expect(barb).toBeDefined();
    expect(calcSellRefund(barb!.cost)).toBe(Math.floor(20 * 0.5)); // 10
  });

  it('BRIK wall sell gives 50% refund (25 credits)', () => {
    const brik = PRODUCTION_ITEMS.find(p => p.type === 'BRIK');
    expect(brik).toBeDefined();
    expect(calcSellRefund(brik!.cost)).toBe(Math.floor(50 * 0.5)); // 25
  });
