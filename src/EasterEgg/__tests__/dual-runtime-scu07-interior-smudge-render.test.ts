/**
 * Dual-runtime visual regression for interior-theatre CellClass smudges.
 *
 * SCU07EA creates scorch/crater CellClass state in the central firefield, but
 * the original C++ SmudgeTypeClass::Init(INTERIOR) asks for SC*.INT/CR*.INT.
 * Those files are absent in the shipped data, so the smudge state remains but
 * Draw_It has null ImageData and renders nothing. TS must not fall back to the
 * temperate smudge sheets in interior missions.
 */
...
async function wasmFirefieldSmudges(adapter: unknown) {
  return adapterPage(adapter).evaluate(() => {
    const module = (window as any).Module;
    const state = JSON.parse(module.ccall('agent_get_state', 'string', [], []));
    return (state.smudges ?? [])
      .filter((s: any) => s.cx >= 97 && s.cx <= 100 && s.cy >= 44 && s.cy <= 51)
      .map((s: any) => ({ type: String(s.type).toLowerCase(), cx: s.cx, cy: s.cy, data: s.data ?? 0 }))
      .sort((a: any, b: any) => a.cy - b.cy || a.cx - b.cx || a.type.localeCompare(b.type));
  }) as Promise<Array<{ type: string; cx: number; cy: number; data: number }>>;
}

async function tsFirefieldSmudges(adapter: unknown) {
  return adapterPage(adapter).evaluate(() => {
    const game = (window as any).__agentGame;
    return (game.map.smudges ?? [])
      .filter((s: any) => s.cx >= 97 && s.cx <= 100 && s.cy >= 44 && s.cy <= 51)
      .map((s: any) => ({ type: String(s.type).toLowerCase(), cx: s.cx, cy: s.cy, data: s.data ?? 0 }))
      .sort((a: any, b: any) => a.cy - b.cy || a.cx - b.cx || a.type.localeCompare(b.type));
  }) as Promise<Array<{ type: string; cx: number; cy: number; data: number }>>;
...
describe.skipIf(!serverUp)('Dual runtime C++ parity: SCU07 interior smudge rendering', () => {
  beforeAll(async () => {
  ...
  it('keeps firefield smudge state but does not render temperate smudge art in INTERIOR', async () => {
    await withDualScenario('SCU07EA', async (handle) => {
      await handle.ts.syncRngSeed(handle.wasmState.rngState!);
      await stepBoth(handle, 100);

      const [cppSmudges, tsSmudges] = await Promise.all([
        wasmFirefieldSmudges(handle.wasm),
        tsFirefieldSmudges(handle.ts),
      ]);

      expect(cppSmudges.length).toBeGreaterThan(0);
      expect(tsSmudges).toEqual(cppSmudges);

      const wasmPng = await wasmRenderedFrame(handle.wasm);
      const tsPng = await tsRenderedFrame(handle.ts);
      const box = { x: 160, y: 100, w: 120, h: 160 };
