...
describe('Superweapon IQ gate (house.cpp IQSuperWeapons=4)', () => {
  ...
  it('superweapon.ts IQ threshold matches rules.ini [IQ] SuperWeapons=4', () => {
    // The gate is in superweapon.ts:266: if (iq < 4) continue;
...
describe('Aircraft production IQ gate (house.cpp:6239 AI_Aircraft)', () => {
  ...
  it('IQ 4 AI CAN produce aircraft', () => {
    // C++ AI_Aircraft fires on its own interval; TS gates production behind
    // tick > productionInterval (60 for normal). Use tick=61 so the guard passes.
...
describe('Harvester replacement IQ gate (house.cpp IQHarvester=2)', () => {
  ...
  it('IQ 2 AI CAN auto-replace harvesters', () => {
    // C++ AI_Harvester requires IQ >= 2. TS gates production behind
    // tick > productionInterval (60 for normal). Use tick=61 so the guard passes.
    const ctx = makeMockAIContext({ tick: 61 });
    const house = House.USSR;
...
describe('AREA_GUARD mission requires IQ >= 4 (house.cpp IQGuardArea=4)', () => {
  ...
  it('IQ 4 spawnAIUnit uses AREA_GUARD as spawn mission', () => {
    // Test spawnAIUnit directly to confirm the mission parameter propagates
    // (updateAIProduction may override to MOVE for staging areas)
    const ctx = makeMockAIContext({ tick: 0 });
    const house = House.USSR;
    ctx.houseIQs.set(house, 4);
    ctx.houseCredits.set(house, 5000);
    addAIHouse(ctx, house, { iq: 4, productionEnabled: true });

    ctx.structures.push(makeStructure('TENT', house, 45, 45));

    const unit = spawnAIUnit(ctx, house, UnitType.I_E1, 'TENT', Mission.AREA_GUARD);
    expect(unit).not.toBeNull();
    expect(unit!.mission).toBe(Mission.AREA_GUARD);
  });

  it('IQ 3 spawnAIUnit uses GUARD (not AREA_GUARD) as spawn mission', () => {
    const ctx = makeMockAIContext({ tick: 0 });
    const house = House.USSR;
    ctx.houseIQs.set(house, 3);
    ctx.houseCredits.set(house, 5000);
    addAIHouse(ctx, house, { iq: 3, productionEnabled: true });

    ctx.structures.push(makeStructure('TENT', house, 45, 45));

    // At IQ 3, the AI production code passes GUARD instead of AREA_GUARD
    const unit = spawnAIUnit(ctx, house, UnitType.I_E1, 'TENT', Mission.GUARD);
    expect(unit).not.toBeNull();
