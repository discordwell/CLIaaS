/**
 * Dual-runtime visual regression for damaged repair facility frame selection.
 *
 * SCU09 starts with a damaged Soviet FIX. C++ BuildingClass::Shape_Number adds
 * the largest registered bdata.cpp animation span (7 for STRUCT_REPAIR) to the
 * idle stage. TS previously used a later repair-pad state with a bright center.
 */
...
describe.skipIf(!serverUp)('Dual runtime C++ parity: SCU09 damaged repair facility', () => {
  beforeAll(async () => {
  ...
  it('renders the damaged idle FIX frame from the C++ largest-animation-span rule', async () => {
    await withDualScenario('SCU09EA', async (handle) => {
      await handle.ts.syncRngSeed(handle.wasmState.rngState!);
      await stepBoth(handle, 1);

      const cppFix = await adapterPage(handle.wasm).evaluate(() => {
        const module = (window as any).Module;
        const state = JSON.parse(module.ccall('agent_get_state', 'string', [], []));
        return (state.logicLayer ?? [])
          .filter((row: any[]) => row[1] === 'FIX' && row[3] === 104 && row[4] === 96)
          .map((row: any[]) => ({
            logicIndex: row[0],
            type: row[1],
            house: row[2],
      ...
      expect(cppFix).toEqual([
        { logicIndex: 211, type: 'FIX', house: 'USSR', cx: 104, cy: 96, hp: 350, maxHp: 800, missionTimer: 43 },
      ]);

      const tsFix = await adapterPage(handle.ts).evaluate(() => {
        const game = (window as any).__agentGame;
        return (game.structures ?? [])
          .filter((s: any) => s.type === 'FIX' && s.cx === 104 && s.cy === 96)
          .map((s: any) => ({
            type: s.type,
            house: s.house,
            cx: s.cx,
      ...
      expect(tsFix).toEqual([
        { type: 'FIX', house: 'USSR', cx: 104, cy: 96, hp: 350, maxHp: 800, missionTimer: 43 },
      ]);
      ...
      expectCropMatch(wasmPng, tsPng, { x: 360, y: 238, w: 50, h: 35 });
