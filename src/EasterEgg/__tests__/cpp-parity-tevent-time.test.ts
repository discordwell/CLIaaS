...
describe('TEVENT_TIME (type=13) — C++ parity', () => {
  ...
  it('returns true when exactly requiredTicks have elapsed', () => {
    const event = timeEvent(5);
    const requiredTicks = 5 * TIME_UNIT_TICKS;
  ...
  it('event.data=0 returns true at tick 0', () => {
    const event = timeEvent(0);
    // gameTick == triggerStartTick, so elapsed = 0, required = 0.
  ...
  it('triggerStartTick offsets the countdown correctly', () => {
    ...
    expect(checkTriggerEvent(event, stateBefore)).toBe(true);
