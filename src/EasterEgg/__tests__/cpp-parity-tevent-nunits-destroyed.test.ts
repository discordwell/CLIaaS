 ...
 *
 * C++ behavior: returns HouseClass::UnitsLost for the trigger's house >= event.data.
 * The trigger fires when the trigger owner's house has lost at least N units,
...
function unitsLost(count: number, house = 1): Map<number, number> {
  return new Map([[house, count]]);
}
...
describe('TEVENT_NUNITS_DESTROYED (type=16) — C++ parity', () => {
  ...
  it('returns false when kills < threshold', () => {
    const state = createState({ unitsLostByHouse: unitsLost(4) });
    expect(checkTriggerEvent(makeEvent(5), state)).toBe(false);
  });

  it('returns true when kills == threshold', () => {
    const state = createState({ unitsLostByHouse: unitsLost(5) });
    expect(checkTriggerEvent(makeEvent(5), state)).toBe(true);
  });

  it('returns true when kills > threshold', () => {
    const state = createState({ unitsLostByHouse: unitsLost(10) });
    expect(checkTriggerEvent(makeEvent(5), state)).toBe(true);
  });
  ...
  it('threshold=0 with positive kills returns true', () => {
    const state = createState({ unitsLostByHouse: unitsLost(3) });
    expect(checkTriggerEvent(makeEvent(0), state)).toBe(true);
  });

  it('returns false when kills=0 and threshold > 0', () => {
    const state = createState({ unitsLostByHouse: unitsLost(0) });
    expect(checkTriggerEvent(makeEvent(1), state)).toBe(false);
  });

  it('handles large threshold values', () => {
    const state = createState({ unitsLostByHouse: unitsLost(999) });
    expect(checkTriggerEvent(makeEvent(1000), state)).toBe(false);

    const state2 = createState({ unitsLostByHouse: unitsLost(1000) });
    expect(checkTriggerEvent(makeEvent(1000), state2)).toBe(true);
  });

  it('threshold=1 requires at least one kill', () => {
    expect(checkTriggerEvent(makeEvent(1), createState({ unitsLostByHouse: unitsLost(0) }))).toBe(false);
    expect(checkTriggerEvent(makeEvent(1), createState({ unitsLostByHouse: unitsLost(1) }))).toBe(true);
    expect(checkTriggerEvent(makeEvent(1), createState({ unitsLostByHouse: unitsLost(2) }))).toBe(true);
  });
  ...
  it('SCA02EA los1 (threshold=8) should NOT fire at tick 0 with fresh killCount', () => {
    ...
    const state = createState({ unitsLostByHouse: unitsLost(0), gameTick: 0 });
    expect(checkTriggerEvent(makeEvent(8), state)).toBe(false);
  });

  it('SCA02EA los1 fires only when 8+ kills achieved in current mission', () => {
    expect(checkTriggerEvent(makeEvent(8), createState({ unitsLostByHouse: unitsLost(7) }))).toBe(false);
    expect(checkTriggerEvent(makeEvent(8), createState({ unitsLostByHouse: unitsLost(8) }))).toBe(true);
  });
});
