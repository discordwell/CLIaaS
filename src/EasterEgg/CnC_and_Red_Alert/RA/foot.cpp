				...
				if (obj != this && obj->What_Am_I() == RTTI_INFANTRY) {
					...
					if (inf->NavCom == NavCom && inf->Path[0] != FACING_NONE) {
						...
						if (Path[0] != FACING_NONE) {
							skip_path = true;
							// DEBUG: log path copy
							if (((InfantryClass*)this)->Class->Type == INFANTRY_E1) {
								int p0 = (Path[0] == -1) ? 9 : Path[0];
								int p1 = (Path[1] == -1) ? 9 : Path[1];
								int p2 = (Path[2] == -1) ? 9 : Path[2];
								int p3 = (Path[3] == -1) ? 9 : Path[3];
								int p4 = (Path[4] == -1) ? 9 : Path[4];
								int p5 = (Path[5] == -1) ? 9 : Path[5];
								extern void agent_debug_log(int, int, int, int, int, int, int, int);
								agent_debug_log(70000, Cell_X(Coord_Cell(Coord)), Cell_Y(Coord_Cell(Coord)),
									Cell_X(cell), Cell_Y(cell), p0*100+p1, p2*100+p3, p4*100+p5);
							}
						}
						break;
					}
...
//					for (MoveType move = MOVE_MOVING_BLOCK; move < maxtype-1; move++) {
		...
		// DEBUG: dump full path for HUNT E1 infantry
		if (Path[0] != FACING_NONE && What_Am_I() == RTTI_INFANTRY &&
			((InfantryClass*)this)->Class->Type == INFANTRY_E1 &&
			Coord_XCell(Coord) >= 42 && Coord_XCell(Coord) <= 62) {
			CELL src = Coord_Cell(Coord);
			extern void agent_debug_log(int, int, int, int, int, int, int, int);
			// Encode path entries as: 80000 + path[0]*1000 + path[1]*100 + path[2]*10 + path[3]
			int p0 = (Path[0] == -1) ? 9 : Path[0];
			int p1 = (Path[1] == -1) ? 9 : Path[1];
			int p2 = (Path[2] == -1) ? 9 : Path[2];
			int p3 = (Path[3] == -1) ? 9 : Path[3];
			int p4 = (Path[4] == -1) ? 9 : Path[4];
			int p5 = (Path[5] == -1) ? 9 : Path[5];
			agent_debug_log(80000, Cell_X(src), Cell_Y(src), Cell_X(cell), Cell_Y(cell),
				p0*100+p1, p2*100+p3, p4*100+p5);
		}
...
//	if (!Target_Legal(TarCom) && !House->IsPlayerControl && !House->IsHuman) {
	...
	}
	{
		...
		int __mm_saved = g_rng_source_tag;
		g_rng_source_tag = 60010; // Mission_Move Random_Pick(0,2)
		int __mm_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __mm_saved;
		return(MissionControl[Mission].Normal_Delay() + __mm_jitter);
	}
}
...
int FootClass::Mission_Capture(void)
{
	...
	*/
	...
	{
		extern int g_rng_source_tag;
		int __mc_saved = g_rng_source_tag;
		g_rng_source_tag = 60020; // Mission_Capture Random_Pick(0,2)
		int __mc_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __mc_saved;
		return(MissionControl[Mission].Normal_Delay() + __mc_jitter);
	}
}
...
int FootClass::Mission_Attack(void)
{
	...
	{
		extern int g_rng_source_tag;
		int __ma_saved = g_rng_source_tag;
		g_rng_source_tag = 60030; // Mission_Attack Random_Pick(0,2)
		int __ma_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __ma_saved;
		return(MissionControl[Mission].Normal_Delay() + __ma_jitter);
	}
}
...
int FootClass::Mission_Guard(void)
{
	...
	{
		extern int g_rng_source_tag;
		int __mg_saved = g_rng_source_tag;
		// Per-type tag: 60040 general, 60041 vessel DD/PT, 60042 vessel CA, 60043 infantry E1/E3
		int __mg_tag = 60040;
		if (What_Am_I() == RTTI_VESSEL) __mg_tag = 60041; // vessel Mission_Guard
		else if (What_Am_I() == RTTI_INFANTRY) __mg_tag = 60043; // infantry Mission_Guard
		g_rng_source_tag = __mg_tag;
		int __mg_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __mg_saved;
		return(dtime + __mg_jitter);
	}
}
...
int FootClass::Mission_Hunt(void)
{
	...
	{
		extern int g_rng_source_tag;
		int __mh_saved = g_rng_source_tag;
		g_rng_source_tag = 60050; // Mission_Hunt Random_Pick(0,2)
		int __mh_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __mh_saved;
		return(MissionControl[Mission].Normal_Delay() + __mh_jitter);
	}
}
...
int FootClass::Mission_Guard_Area(void)
{
	...
	*/
	...
	{
		extern int g_rng_source_tag;
		int __mga_saved_tag = g_rng_source_tag;
		g_rng_source_tag = 30000; // Mission_Guard_Area Random_Pick(1,5)
		int __mga_jitter = Random_Pick(1, 5);
		g_rng_source_tag = __mga_saved_tag;
		return(dtime + __mga_jitter);
	}
}
			...
			*/
			if (Is_Allowed_To_Retaliate(source)) {
				...
				if (In_Range(source, primary) || !House->IsHuman) {
					{ extern int g_tarcom_tag; g_tarcom_tag = 70001; }
					Assign_Target(source->As_Target());
					{ extern int g_tarcom_tag; g_tarcom_tag = 0; }
				}
...
int FootClass::Mission_Enter(void)
{
	...
	*/
	...
	{
		extern int g_rng_source_tag;
		int __men_saved = g_rng_source_tag;
		g_rng_source_tag = 60060; // Mission_Enter Random_Pick(0,2)
		int __men_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __men_saved;
		return(MissionControl[Mission].Normal_Delay() + __men_jitter);
	}
}
...
int FootClass::Mission_Retreat(void)
{
	...
	{
		extern int g_rng_source_tag;
		int __mret_saved = g_rng_source_tag;
		g_rng_source_tag = 60070; // Mission_Retreat Random_Pick(0,2)
		int __mret_jitter = Random_Pick(0, 2);
		g_rng_source_tag = __mret_saved;
