...
void LogicClass::AI(void)
{
	...
	// Session 14 diagnostic: frame-boundary marker. tag 4000000+Frame fires at LogicClass::AI entry.
	{
		extern void agent_debug_log(int a, int b, int c, int d, int e, int f, int g, int h);
		if (Frame < 6) {
			agent_debug_log(4000000 + Frame, 0, 0, 0, 0, 0, 0, 0);
		}
	}
	...
	*/
	...
	extern int g_rng_entity_tag;
	g_rng_source_tag = 20; // Do_Fade_AI
	Scen.Do_Fade_AI();
	/*
	**	Handle any general timer trigger events.
	*/
	g_rng_source_tag = 21; // LogicTrigger processing
	...
	*/
	...
	{
		extern void agent_debug_log(int a, int b, int c, int d, int e, int f, int g, int h);
		if (Frame < 6) { agent_debug_log(4100000 + Frame, 0, 0, 0, 0, 0, 0, 0); }
	}
	...
	{
		extern void agent_debug_log(int a, int b, int c, int d, int e, int f, int g, int h);
		if (Frame < 6) { agent_debug_log(4200000 + Frame, 0, 0, 0, 0, 0, 0, 0); }
	}
	...
	*/
	...
	{
		extern void agent_debug_log(int a, int b, int c, int d, int e, int f, int g, int h);
		if (Frame < 6) { agent_debug_log(4300000 + Frame, 0, 0, 0, 0, 0, 0, 0); }
	}
	/*
	...
	*/
	for (index = 0; index < Count(); index++) {
		...
		// Set source tag: type * 1000 + Logic index (unique per object per tick)
		switch (obj->What_Am_I()) {
			case RTTI_INFANTRY: g_rng_source_tag = 10000 + index; break;
			case RTTI_UNIT:     g_rng_source_tag = 11000 + index; break;
			case RTTI_BUILDING: g_rng_source_tag = 12000 + index; break;
			case RTTI_AIRCRAFT: g_rng_source_tag = 13000 + index; break;
			case RTTI_VESSEL:   g_rng_source_tag = 14000 + index; break;
			case RTTI_BULLET:   g_rng_source_tag = 15000 + index; break;
			case RTTI_ANIM:     g_rng_source_tag = 16000 + index; break;
			default:            g_rng_source_tag = 2000 + index; break;
		}
		// Entity tag stays SET for the duration of this entity's processing,
		// even when granular callers (foot.cpp/infantry.cpp/etc.) override
		// g_rng_source_tag. This lets RNG logs identify WHICH entity a call
		// belongs to, regardless of the granular action (30001, 60043, etc.).
		g_rng_entity_tag = g_rng_source_tag;
	...
	*/
	g_rng_source_tag = 3; // Map.Logic
	Map.Logic();

	/*
	**	Factory processing is performed.
	*/
	g_rng_source_tag = 4; // Factory AI
	...
	*/
	...
	else
	{
		for (HousesType house = HOUSE_FIRST; house < HOUSE_COUNT; house++) {
			HouseClass * hptr = HouseClass::As_Pointer(house);
			if (hptr && hptr->IsActive) {
				g_rng_source_tag = 5; // House AI preamble
				hptr->AI();
			}
		}
	}
