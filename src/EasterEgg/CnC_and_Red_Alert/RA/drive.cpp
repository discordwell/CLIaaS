...
void DriveClass::Scatter(COORDINATE threat, bool forced, bool nokidding)
{
	...
	*/
	...
	if ((What_Am_I() != RTTI_UNIT || !((UnitClass *)this)->IsDumping) && (!Target_Legal(NavCom) || (nokidding && !IsRotating))) {
		...
		int __sc_saved = g_rng_source_tag;
		bool __sc_trigger;
		if (!Target_Legal(TarCom) || forced) {
			__sc_trigger = true;
		} else {
			g_rng_source_tag = 60080; // DriveClass::Scatter Random_Pick(1,4) gate
			__sc_trigger = (Random_Pick(1, 4) == 1);
			g_rng_source_tag = __sc_saved;
		}
		if (__sc_trigger) {
			FacingType	toface;
			FacingType	newface;
			...
			if (threat != 0) {
				...
				g_rng_source_tag = 60081; // Scatter face offset (threat)
				toface = toface + FacingType(Random_Pick(0, 2)-1);
				g_rng_source_tag = __sc_saved;
			} else {
				toface = Dir_Facing(PrimaryFacing.Current());
				g_rng_source_tag = 60082; // Scatter face offset (no threat)
				toface = toface + FacingType(Random_Pick(0, 2)-1);
				g_rng_source_tag = __sc_saved;
			}
