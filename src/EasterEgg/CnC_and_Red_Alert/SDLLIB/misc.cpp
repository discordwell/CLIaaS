...
static uint8_t Fade_Gun(uint8_t orig, uint8_t target, long frac)
{
    const int16_t delta = static_cast<int16_t>(orig) - static_cast<int16_t>(target);
    const int16_t product = static_cast<int16_t>(delta * static_cast<int16_t>(frac >> 1));
    const uint16_t doubled = static_cast<uint16_t>(static_cast<uint16_t>(product) << 1);
    const uint8_t high_byte = static_cast<uint8_t>(doubled >> 8);
    return static_cast<uint8_t>(orig - high_byte);
}

void Delay(int duration)
{
#ifdef __EMSCRIPTEN__
    extern int g_agent_harness_mode;
...
void *Build_Fading_Table(void const *palette, void *dest, long int color, long int frac)
{
    ...
    targetgreen = pal8[color * 3 + 1];
    targetblue = pal8[color * 3 + 2];
    ...
    *dptr++ = 0;
    ...
    for(int remap_index = 1; remap_index < 256; remap_index++)
    {
        ...
        idealred = Fade_Gun(origred, targetred, frac);
        idealgreen = Fade_Gun(origgreen, targetgreen, frac);
        idealblue = Fade_Gun(origblue, targetblue, frac);
