    ...
    const demoPlugins: PluginManifest[] = [
      {
        ...
        name: 'GitHub Sync',
        version: '1.2.0',
        description: 'Automatically creates GitHub issues from support tickets and syncs status updates bi-directionally.',
        author: 'CLIaaS Team',
        hooks: ['ticket.created', 'ticket.updated', 'ticket.resolved'],
        actions: [
          { id: 'create-issue', name: 'Create GitHub Issue', description: 'Creates a new GitHub issue linked to this ticket' },
          { id: 'link-pr', name: 'Link Pull Request', description: 'Links a GitHub PR to this ticket for tracking' },
        ],
        enabled: true,
        installedAt: new Date(Date.now() - 30 * 86400000).toISOString(),
        config: { repo: 'org/repo', labelPrefix: 'support:', autoCreate: true },
      },
      {
        id: 'pagerduty-alerts',
        name: 'PagerDuty Alerts',
        version: '2.0.1',
        description: 'Triggers PagerDuty incidents when SLA is breached or urgent tickets are created. Supports on-call rotation.',
        author: 'CLIaaS Team',
        hooks: ['sla.breached', 'ticket.created'],
        actions: [
          { id: 'trigger-incident', name: 'Trigger Incident', description: 'Creates a PagerDuty incident for this ticket' },
          { id: 'acknowledge', name: 'Acknowledge Incident', description: 'Acknowledges the linked PagerDuty incident' },
        ],
        enabled: true,
        installedAt: new Date(Date.now() - 14 * 86400000).toISOString(),
        config: { serviceId: 'P1234ABC', urgencyFilter: ['urgent', 'high'], autoTrigger: true },
      },
      {
        id: 'stripe-context',
        name: 'Stripe Context',
        version: '1.0.3',
        description: 'Enriches tickets with Stripe customer data including subscription status, payment history, and MRR.',
        author: 'CLIaaS Team',
        hooks: ['ticket.created', 'message.created'],
        actions: [
          { id: 'lookup-customer', name: 'Lookup Stripe Customer', description: 'Fetches Stripe customer data for the requester' },
          { id: 'view-invoices', name: 'View Recent Invoices', description: 'Shows recent invoices for the customer' },
        ],
        enabled: true,
        installedAt: new Date(Date.now() - 7 * 86400000).toISOString(),
        config: { lookupBy: 'email', showMRR: true, invoiceLimit: 5 },
      },
    ];
